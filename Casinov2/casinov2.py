...
class Casino:
    ...
    def magic(self):
        ...
        while True:
            ...
            if number > random_number:
                ...
                if self.player_data['magic']['record'] == None:
                ...
                random_number = random.randint(1, 100)
                counter = 0
            # self.player_data['points'] = point
            # self.player_data['magic']['games'] = counter_pusk
            # self.player_data['magic']['wins'] = pusk_plus
            # self.player_data['magic']['record'] = record_counter
            # self.player_data['magic']['koef'] = payoff_ratio
    ...
    def blackjack(self):
        ...
        # от 1 до 8 колод
        count = 0
        # self.player_data['blackjack']['games'] = count_play
        # self.player_data['blackjack']['wins'] = play_win
        # self.player_data['blackjack']['koef'] = payoff_rat
        ...
        while True:
            ...
            if choice == 'y':
                ...
                count += current
                if count > 21:
                    self.player_data['points'] -= rate  # отнимаем ставку в случе проиграша
                    print('Увы перебор')
                    return self.main_menu()
                elif count == 21:
    ...
    def statistic(self):
        ...
        print('Количество очков:', self.player_data['points'])
        print('\nMagic')
        print('Всего игр сыграно: ', self.player_data['magic']['games'])
        print('Выиграно: ', self.player_data['magic']['wins'])
        if self.player_data['magic']['games'] != 0:
            print('Коэфициент выграшей: ', self.player_data['magic']['wins']/self.player_data['magic']['games'])
        else:
            print('Коэфициент выграшей: ', 0.0)
        if self.player_data['magic']['record'] != None:
            print('Рекордное количество попыток: ', self.player_data['magic']['record'])
        else:
            print('Рекордное количество попыток: ', 0)
        print('\nBlackjack')
        print('Всего игр сыграно: ', self.player_data['blackjack']['games'])
        print('Выиграно: ', self.player_data['blackjack']['wins'])
        if self.player_data['blackjack']['games'] != 0:
            print('Коэфициент выграшей: ', self.player_data['blackjack']['wins']/self.player_data['blackjack']['games'])
        else:
            print('Коэфициент выграшей: ', 0.0)
